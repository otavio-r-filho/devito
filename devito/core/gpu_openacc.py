...
from devito.passes.iet import (DataManager, Storage, OpenACCizer, ParallelIteration,
...
class DeviceOpenACCizer(OpenACCizer):
    ...
    lang = dict(OpenACCizer.lang)
    lang.update({
        'map-enter-to': lambda i, j:
            c.Pragma('acc enter data copyin(%s%s)' % (i, j)),
        'map-enter-alloc': lambda i, j:
            c.Pragma('omp create data (%s%s)' % (i, j)),
        'map-update': lambda i, j:
            c.Pragma('acc update device(%s%s)' % (i, j)),
        'map-release': lambda i, j:
            c.Pragma('acc data copyout(%s%s)' % (i, j)),
...
class DeviceDataManager(DataManager):
    ...
    def _alloc_array_on_high_bw_mem(self, obj, storage):
        ...
        alloc = c.Collection([alloc, DeviceOpenACCizer._map_alloc(obj)])
        free = c.Collection([DeviceOpenACCizer._map_delete(obj), free])
    ...
    def _map_function_on_high_bw_mem(self, obj, storage, read_only=False):
        ...
        alloc = DeviceOpenACCizer._map_to(obj)

        if read_only is False:
            free = c.Collection([DeviceOpenACCizer._map_update(obj),
                                 DeviceOpenACCizer._map_release(obj)])
        else:
            free = DeviceOpenACCizer._map_delete(obj)
...
class DeviceOpenACCNoopOperator(OperatorCore):

    @classmethod
    def _build(cls, *args, **kwargs):
        # Strictly unneccesary, but make it clear that this Operator *will*
        # generate OpenACC code, bypassing any `openacc=False` provided in
        # input to Operator
        kwargs['options'].pop('openmp')

        return super(DeviceOpenACCNoopOperator, cls)._build(*args, **kwargs)
    ...
    def _specialize_iet(cls, graph, **kwargs):
        ...
        DeviceOpenACCizer().make_parallel(graph)
